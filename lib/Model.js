...
Model.statics = {
  ...
  get: function(id, callback) {
    ...
    if (id instanceof Array) {
      ...
      for (var i = 0; i < id.length; i++) {
        multi.hgetall(this.getKey(id[i]));
      }
      multi.exec(cb);
    } else {
      this.client.hgetall(this.getKey(id), cb);
    }
  },
  /**
  ...
  /**
   * Build one or more objects from an hgetall or a multi-wrapped hgetall.
   *
  ...
  hydrate: function(id, result) {
    if (result instanceof Array) {
      ...
      for (var i = 0; i < result.length; i++) {
        results.push(this.hydrate(id[i], result[i]));
      }
      return results;
    } else {
      var mdl = new this();
